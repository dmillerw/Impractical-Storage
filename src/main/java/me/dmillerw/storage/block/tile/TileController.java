...
public class TileController extends TileCore implements ITickable {
                ...
                if (CommonProxy.blockUpdateBatch == -1) {
                    for (int i = 0; i < blockQueue.size(); i++) {
                        ...
                        if (!setBlock(element.slot, element.itemStack)) {
                            InventoryHelper.spawnItemStack(worldObj, pos.getX(), pos.getY(), pos.getZ(), element.itemStack.copy());
                        }
                    }
                } else {
                    ...
                    for (int z = -1; z <= zLength; z++) {
                        for (int x = -1; x <= xLength; x++) {
                            if (y == -1 || y == height || z == -1 || z == zLength || x == -1 || x == xLength) {
                                ...
                                if (worldObj.getBlockState(pos).getBlock() == ModBlocks.controller_interface) {
    ...
    public void updateOffset(int x, int y, int z) {
        ...
        this.updateRawBounds(worldObj.getBlockState(pos).getValue(BlockController.FACING), rawX, rawY, rawZ);
    }
    ...
    public boolean isInventoryEmpty() {
        boolean empty = true;
        for (ItemStack stack : inventory) if (stack != null) empty = false;
        return empty;
    }
    ...
    private void shiftInventory() {
        ItemStack[] shifted = new ItemStack[totalSize];
